...
=======
model(np.zeros([1]+list(train_images.shape[1:]), dtype = np.float32), training = True)
    ...
    for epoch in range(train_epoch):
        for images, labels in train_ds:
            ...
            if step % should_log == 0:
                ...
                train_time = time.time()
        tf.summary.scalar('Categorical_loss/train', train_loss.result(), step=epoch)
        tf.summary.scalar('Accuracy/train', train_accuracy.result()*100, step=epoch)
            
        for test_images, test_labels in test_ds:
            test_images = pre_processing(test_images, is_training = False)
            test_step(test_images, test_labels)
        tf.summary.scalar('Categorical_loss/test', test_loss.result(), step=epoch)
        tf.summary.scalar('Accuracy/test', test_accuracy.result()*100, step=epoch)
